...
static void common_params_fit_impl(
    ...
    // typdigital: NOTE - a partial-layer rebalance pass (moving dense-only
    // layers toward the fastest device) was tested here and REVERTED: it
    // regressed GLM-5.3-Flash 5.4 -> 1.4 tok/s on the 2-GPU layout (the
    // reshuffled distribution costs more in graph-split syncs than it
    // gains). The original greedy fill + dense-to-full conversion is the
    // measured optimum for this fleet. Do not re-add without a bisect.
    set_ngl_tensor_split_tbo(ngl_per_device, overflow_bufts, *mparams);
}
